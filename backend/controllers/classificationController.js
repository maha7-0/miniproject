...
const userId = req.user.id;
  ...
  try {
    ...
    res.status(500).json({ success: false, message: 'Classification error', error: err.message });
  }
};
  ...
  try {
    ...
    const enrichedRecords = await Promise.all(
      records.map(async (record) => {
        const diatomClass = await DiatomClass.findOne({ name: record.predictedClass });
        return {
          ...record.toObject(),
